...
function renderTask(taskObj) {
  ...
  for (let i = 1; i < colspan; i++) {
    const curr = startPos.index + i;
    const currHour = startPos.hour + Math.floor(curr / minutes.length);
    const currMinIdx = curr % minutes.length;
    const cellToHide = tbody.querySelector(
      `tr:nth-child(${currHour - startHour + 1}) td:nth-child(${currMinIdx + 2})`
    );
...
function renderTaskList(){
  ...
  tasks.forEach((t,i) => {
    ...
    editBtn.onclick = () => {
      const newTask = prompt("할 일 수정:", t.task); if (newTask === null) return;
